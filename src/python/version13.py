...
def genetic_algorithm(r_seed, n_strings, length, n_epochs, n_rounds, m_rate, c_rate):
    ...
    best_fitness, best_string = -1.0, numpy.empty(length, numpy.ushort)
    # seed the random number generator
    rng = numpy.random.default_rng(r_seed)
    # initialize the first population of bitstring
    bitstrings_pop = rng.integers(0, 1, (n_strings, length), numpy.ushort, True)
    # preallocate memory for the children we will create
    bitstrings_children = numpy.empty((n_strings, length), numpy.ushort)
    # empty array for all fitness scores
    fitness_pop = numpy.zeros(n_strings, numpy.ushort)
    ...
    parents = numpy.empty(n_strings, numpy.ushort)
    # run the algorithm
    for epoch in range(n_epochs):
        # calculate fitness for current population (onemax)
        numpy.sum(bitstrings_pop, axis=1, dtype=numpy.ushort, out=fitness_pop)
        ...
        torn_ixs = rng.integers(0, n_strings, (n_strings, n_rounds), numpy.ushort)
        ...
        cross_points = rng.integers(1, length-1, n_strings//2, numpy.uintc)
